...
function initButtons() {
    let rippleEls: NodeListOf<Element> = document.getElementsByClassName("ripple-ink");
...
function initInputs() {
    let materializedInputs = document.getElementsByClassName("md-input-container");
    for (let i = 0; i < materializedInputs.length; i++) {
        let input = materializedInputs[i].querySelector("input");
        if (input.value != "")
